...
import {
  MessageCircle, ExternalLink, Loader2, MessageSquarePlus, Send, Search, Trash2,
} from 'lucide-react'
import { useAuth } from '../../context/AuthContext'
import { STATUS_CHAT_SUGGESTIONS, useStatusChat } from '../../hooks/useStatusChat'
import clsx from 'clsx'

export default function StatusChatWidget({
  variant = 'page',
  enabled = true,
  showHistory = true,
}) {
  ...
  const { user } = useAuth()
  const chat = useStatusChat({ enabled })
  const {
    sessions,
    activeId,
    messages,
  ...
  const suggestions = STATUS_CHAT_SUGGESTIONS
    ...
    >
      ...
      >
        {loadingSessions ? (
          ...
          !isPanel && <p className="text-xs text-slate-400 px-2">{t('status_chat.no_history')}</p>
        ) : (
          sessions.slice(0, isPanel ? 5 : undefined).map((s) => (
            ...
            >
              <span className="truncate font-medium">{s.title || t('status_chat.untitled')}</span>
              {!isPanel && (
                <span
                  role="button"
                  tabIndex={0}
  ...
  const chatBody = (
    <div className={clsx('flex flex-col min-h-0', isPanel ? 'flex-1' : 'flex-1 card')}>
      <div className="flex items-center gap-2 px-3 py-2.5 border-b border-slate-100 dark:border-slate-800 shrink-0">
        <div className="w-8 h-8 rounded-lg bg-teal-100 dark:bg-teal-900/40 flex items-center justify-center text-teal-600 dark:text-teal-300">
          <MessageCircle size={18} />
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">
            {t('status_chat.assistant_name')}
          </p>
          {user?.role && (
            <p className="text-[10px] text-slate-500 truncate">
              {t('status_chat.signed_in_as', { role: user.role })}
            </p>
          )}
        </div>
        {isPanel && (
      ...
      >
        {messages.length === 0 && !loading && (
          <div className={clsx('text-center', isPanel ? 'py-4' : 'py-8 max-w-lg mx-auto')}>
            <Search className="mx-auto text-teal-400 mb-2" size={isPanel ? 24 : 32} />
            <p className="text-xs text-slate-600 dark:text-slate-300 mb-3">
              {t('status_chat.welcome_short')}
            </p>
            <div className="flex flex-wrap gap-1.5 justify-center">
              {suggestions.slice(0, isPanel ? 2 : 4).map((q) => (
                <button
                  ...
                  className="text-[10px] px-2 py-1 rounded-full border border-teal-200 dark:border-teal-800 text-teal-700 dark:text-teal-300 hover:bg-teal-50 dark:hover:bg-teal-900/20 text-left"
                >
        ...
        {loading && (
          <div className="flex items-center gap-2 text-xs text-teal-600 dark:text-teal-300">
            <Loader2 size={16} className="animate-spin shrink-0" />
            {t('status_chat.thinking')}
          </div>
        )}
        <div ref={bottomRef} />
      </div>
      ...
      >
        <input
          ...
          placeholder={t('status_chat.placeholder')}
          value={input}
