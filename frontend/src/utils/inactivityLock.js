 ...
 *  via the dedicated /auth/lock page); a user with no PIN set is fully
 *  logged out instead, since locking would be a dead end.
 *
 *  Session PIN is mandatory system-wide as of 2026-08-21, so this defaults
 *  to a real value rather than "never" — an indefinitely open session is a
 *  real exposure on confidential submissions. Each user can still adjust it
 *  (or turn it off) in Account settings. */
