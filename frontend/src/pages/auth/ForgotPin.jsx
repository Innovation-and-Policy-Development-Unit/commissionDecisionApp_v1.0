...
/** Forgot your session PIN — separate from Forgot Password. The PIN is
 * stored hashed and can't be recovered, only replaced, so this sends a
 * reset link the same way password reset does. */
export default function ForgotPin() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      await api.post('/auth/pin-reset/request/', { email: email.trim() })
      setSent(true)
    } catch (err) {
          ...
          >
            {!sent ? (
              <>
                ...
                <h1 className="text-xl font-bold text-center text-slate-900 mb-1">Forgot your PIN?</h1>
                <p className="text-sm text-center text-slate-500 mb-6">
                  Enter the email address linked to your account and we'll send you a link to set
                  a new session PIN. This doesn't change your account password.
                </p>
              ...
              <>
                ...
                <p className="text-sm text-center text-slate-500 mb-6">
                  ...
                  registered, we've sent a PIN reset link. The link expires in 1 hour.
                </p>
