...
import { BarChart3, RefreshCw, AlertCircle, Clock, CheckCircle2, Download, Loader2 } from 'lucide-react'
...
async function exportPdf(aggregates) {
  ...
  // ── Header bar ────────────────────────────────────────────────────────────
  doc.setFillColor(30, 64, 175)          // primary-800 blue
  doc.rect(0, 0, W, 22, 'F')
  ...
  autoTable(doc, {
    ...
    columnStyles:  Object.fromEntries(stats.map((_, i) => [i, { halign: 'center' }])),
  ...
  // ── Cases by family ───────────────────────────────────────────────────────
  sectionHeader(doc, 'Cases by Case Family', y)
  y += 6
  autoTable(doc, {
    startY: y,
  ...
  // ── SLA health ────────────────────────────────────────────────────────────
  sectionHeader(doc, 'SLA Health (All Stages)', y)
  y += 6
  ...
  autoTable(doc, {
    ...
    body:   [
      ['Overdue',   slaOverdue,  pct(slaOverdue)],
      ['At risk',   slaAtRisk,   pct(slaAtRisk)],
      ['On track',  slaOnTrack,  pct(slaOnTrack)],
      ['Completed', slaDone,     pct(slaDone)],
    ],
    theme:  'striped',
    headStyles:  { fillColor: [51, 65, 85], textColor: 255, fontSize: 8 },
    bodyStyles:  { fontSize: 8, textColor: [30, 41, 59] },
    ...
    didParseCell(data) {
      if (data.section === 'body' && data.column.index === 0) {
        const colors = { Overdue: [220, 38, 38], 'At risk': [217, 119, 6], 'On track': [71, 85, 105], Completed: [22, 163, 74] }
  ...
  if (aggregates.byStatus.length) {
    sectionHeader(doc, 'Cases by Status', y)
    y += 6
    autoTable(doc, {
      startY: y,
      head:   [['Status', 'Count']],
  ...
  // ── Senior executive breakdown ────────────────────────────────────────────
  if (aggregates.bySenior.length) {
    if (y > 230) { doc.addPage(); y = 20 }
    sectionHeader(doc, 'Senior Executive Cases', y)
    y += 6
    autoTable(doc, {
      startY: y,
  ...
  doc.save(`OPSC_Compliance_Report_${dateStr}.pdf`)
}
...
export default function ComplianceReports() {
  ...
  const handleExport = async () => {
    ...
    try {
      await exportPdf({
        ...
        byFamily: Object.entries(byFamilyMap)
          .sort((a, b) => b[1].total - a[1].total)
          .map(([, v]) => v),
        sla: { slaOverdue, slaAtRisk, slaOnTrack, slaDone, slaTotal },
        byStatus: STATUS_ORDER
          .filter((s) => byStatusMap[s])
          .map((s) => ({ status: s, count: byStatusMap[s] })),
        bySenior: Object.entries(bySeniorMap)
          .sort((a, b) => b[1] - a[1])
          .map(([key, count]) => ({ label: FAMILY_LABELS[key] || key, count })),
      })
    } finally {
      setExporting(false)
    }
  ...
  return (
    <div>
      <PageHeader
        title="Case Reports"
        subtitle="Aggregate statistics for all compliance cases"
        action={
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <section className="rounded-xl border border-slate-200 dark:border-slate-700 p-5">
            <h3 className="mb-4 flex items-center gap-2 font-semibold text-slate-700 dark:text-slate-200">
              <Clock size={16} /> SLA health (all stages)
            </h3>
            <div className="space-y-3">
              {[
                ...
                <div key={label} className="flex items-center gap-3">
                  {Icon
                    ? <Icon size={15} className={`${cls} shrink-0`} />
                    : <div className="w-[15px] h-[15px] shrink-0 rounded-full bg-slate-400" />
                  }
          ...
          <section className="rounded-xl border border-slate-200 dark:border-slate-700 p-5">
            <h3 className="mb-4 flex items-center gap-2 font-semibold text-slate-700 dark:text-slate-200">
              <BarChart3 size={16} /> Cases by status
            </h3>
            {Object.keys(byStatusMap).length === 0
              ? <p className="text-sm text-slate-400">No cases yet.</p>
              : STATUS_ORDER.filter((s) => byStatusMap[s])
                  .map((s) => (
