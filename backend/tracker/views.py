...
class MinutesViewSet(viewsets.ModelViewSet):
    ...
    # ── Per-agenda-item visibility locks (restrict / share / request access) ──

    def _require_minute_access_manager(self, request):
        from .minutes_access import user_can_manage_minute_access

        if not user_can_manage_minute_access(request.user):
            raise PermissionDenied(
                "Only the PSC Secretary or Administrator can restrict or share minute agenda items."
            )

    def _minutes_response(self, request, minutes):
        return Response(MinutesSerializer(minutes, context={"request": request}).data)

    @action(detail=True, methods=["post"], url_path="restrict-item")
    def restrict_item(self, request, pk=None):
        """Lock one agenda item; optionally share it with specific users at once."""
        from .audit import log_action
        from .minutes_access import ensure_block_key, notify_access_granted
        from .models import AgendaAccessGrant, AgendaItemRestriction

        minutes = self.get_object()
        self._require_minute_access_manager(request)
        if minutes.status != MinutesStatus.SIGNED:
            return Response(
                {"detail": "Agenda items can only be restricted after the minutes are endorsed (signed)."},
                status=400,
            )
        ...
        try:
            ...
            return Response({"detail": "Provide the agenda item index to restrict."}, status=400)
        try:
            item_key = ensure_block_key(minutes, index)
        except IndexError:
            return Response({"detail": "Agenda item not found."}, status=400)

        block = (minutes.content or {}).get("agenda_items", [])[index]
        title = (block.get("title") or block.get("submission_ref") or f"Item {index + 1}")[:512]
        reason = (request.data.get("reason") or "").strip()

        restriction, created = AgendaItemRestriction.objects.get_or_create(
            minutes=minutes, item_key=item_key,
            defaults={"item_title": title, "reason": reason, "restricted_by": request.user},
        )
        if not created and reason and reason != restriction.reason:
            restriction.reason = reason
            restriction.save(update_fields=["reason"])

        user_ids = request.data.get("user_ids") or []
        for grantee in User.objects.filter(id__in=user_ids, is_active=True):
            _, grant_created = AgendaAccessGrant.objects.get_or_create(
                restriction=restriction, user=grantee,
                defaults={"granted_by": request.user},
            )
            if grant_created:
                notify_access_granted(restriction, grantee, request.user)

        log_action(
            request, AuditLog.Action.PERMISSION,
            resource_type="Minutes", resource_id=minutes.id,
            resource_label=minutes.meeting.reference_number,
            description=f'Restricted agenda item "{title}" ({item_key}).',
            extra_data={"item_key": item_key, "shared_with": list(user_ids)},
        )
        return self._minutes_response(request, minutes)

    @action(detail=True, methods=["post"], url_path="unrestrict-item")
    def unrestrict_item(self, request, pk=None):
        """Remove the lock from an agenda item (visible to everyone again)."""
        from .audit import log_action

        minutes = self.get_object()
        self._require_minute_access_manager(request)
        restriction = minutes.agenda_restrictions.filter(
            id=request.data.get("restriction_id")
        ).first()
        if restriction is None:
            return Response({"detail": "Restriction not found."}, status=404)

        label = restriction.item_title or restriction.item_key
        restriction.delete()
        log_action(
            request, AuditLog.Action.PERMISSION,
            resource_type="Minutes", resource_id=minutes.id,
            resource_label=minutes.meeting.reference_number,
            description=f'Removed restriction on agenda item "{label}".',
        )
        return self._minutes_response(request, minutes)

    @action(detail=True, methods=["post"], url_path="grant-access")
    def grant_access(self, request, pk=None):
        """Share a locked agenda item with specific users."""
        from .audit import log_action
        from .minutes_access import notify_access_granted
        from .models import AgendaAccessGrant, AgendaAccessRequestStatus
        ...
        granted = []
        for grantee in User.objects.filter(id__in=user_ids, is_active=True):
            _, created = AgendaAccessGrant.objects.get_or_create(
                restriction=restriction, user=grantee,
                defaults={"granted_by": request.user},
            )
            if created:
                granted.append(grantee.username)
                notify_access_granted(restriction, grantee, request.user)
            # A grant settles any pending request from the same user.
            restriction.access_requests.filter(
                requested_by=grantee, status=AgendaAccessRequestStatus.PENDING
            ).update(
                status=AgendaAccessRequestStatus.APPROVED,
        ...
        log_action(
            ...
            resource_label=minutes.meeting.reference_number,
            description=(
                f'Shared agenda item "{restriction.item_title or restriction.item_key}" '
                f"with: {', '.join(granted) or '—'}."
            ),
        )
        return self._minutes_response(request, minutes)
    ...
    def revoke_access(self, request, pk=None):
        """Remove one user from a locked agenda item's allowlist."""
        from .audit import log_action
        ...
        grant = restriction.grants.filter(user_id=request.data.get("user_id")).first()
        if grant is None:
            return Response({"detail": "This user has no access grant."}, status=404)
        ...
        log_action(
            ...
            resource_label=minutes.meeting.reference_number,
            description=(
                f'Revoked {username}\'s access to agenda item '
                f'"{restriction.item_title or restriction.item_key}".'
            ),
        )
        return self._minutes_response(request, minutes)
    ...
    def request_access(self, request, pk=None):
        """Ask the Secretary/Admin for sight of a locked agenda item."""
        from .audit import log_action
        from .minutes_access import notify_access_request, user_can_view_item
        from .models import AgendaAccessRequest, AgendaAccessRequestStatus

        minutes = self.get_object()
        restriction = minutes.agenda_restrictions.filter(
            item_key=request.data.get("item_key")
        ).prefetch_related("grants").first()
        if restriction is None:
            return Response({"detail": "This agenda item is not restricted."}, status=404)
        if user_can_view_item(request.user, restriction):
            return Response({"detail": "You already have access to this agenda item."}, status=400)
        if restriction.access_requests.filter(
            requested_by=request.user, status=AgendaAccessRequestStatus.PENDING
        ).exists():
            return Response({"detail": "Your access request is already pending."}, status=400)

        AgendaAccessRequest.objects.create(
            restriction=restriction,
            requested_by=request.user,
            message=(request.data.get("message") or "").strip(),
        )
        notify_access_request(restriction, request.user)
        log_action(
            request, AuditLog.Action.PERMISSION,
            resource_type="Minutes", resource_id=minutes.id,
            resource_label=minutes.meeting.reference_number,
            description=(
                f'Requested access to restricted agenda item '
                f'"{restriction.item_title or restriction.item_key}".'
            ),
        )
        return self._minutes_response(request, minutes)

    @action(detail=True, methods=["post"], url_path="decide-request")
    def decide_request(self, request, pk=None):
        """Approve or deny a pending access request (Secretary/Admin only)."""
        from .audit import log_action
        from .minutes_access import notify_access_decision
        from .models import AgendaAccessGrant, AgendaAccessRequest, AgendaAccessRequestStatus

        minutes = self.get_object()
        self._require_minute_access_manager(request)
        req = (
            AgendaAccessRequest.objects
            .filter(
                id=request.data.get("request_id"),
                restriction__minutes=minutes,
                status=AgendaAccessRequestStatus.PENDING,
            )
            .select_related("restriction", "requested_by")
            .first()
        )
        if req is None:
        ...
        req.status = (
            AgendaAccessRequestStatus.APPROVED if approve else AgendaAccessRequestStatus.DENIED
        )
        ...
        if approve:
            AgendaAccessGrant.objects.get_or_create(
                restriction=req.restriction, user=req.requested_by,
                defaults={"granted_by": request.user},
            )
        notify_access_decision(req)
        log_action(
            request, AuditLog.Action.PERMISSION,
            resource_type="Minutes", resource_id=minutes.id,
            resource_label=minutes.meeting.reference_number,
            description=(
                f"{'Approved' if approve else 'Denied'} {req.requested_by.username}'s access to "
                f'agenda item "{req.restriction.item_title or req.restriction.item_key}".'
            ),
        )
        return self._minutes_response(request, minutes)

    @action(detail=True, methods=["get"], url_path="shareable-users")
    def shareable_users(self, request, pk=None):
        """User picker for sharing a locked item (Secretary/Admin only)."""
        from django.db.models import Q

        self.get_object()
        self._require_minute_access_manager(request)
        q = (request.query_params.get("q") or "").strip()
        users = User.objects.filter(is_active=True).select_related("psc_profile")
        if q:
            users = users.filter(
        ...
        results = []
        for u in users.order_by("first_name", "username")[:20]:
