#!/usr/bin/env python3
"""Generate Form Builder import XML for compliance digitized forms."""
...
FORM_META = {
    "COMP-SMDR": {
        ...
        "pdf": "2-SMDR Submission.pdf",
    },
    "COMP-PAR": {
        "name": "Preliminary Assessment Report",
        "digitized_form_key": "comp_par",
        "pdf": "5-Submission- Preliminary Assessment Report.pdf",
    },
    "COMP-PSDB": {
        "name": "PSDB Order on Determination",
        "digitized_form_key": "comp_psdb",
        "pdf": "3-Submission-PSDB Order on Determination.pdf",
    },
    "COMP-14D": {
        "name": "Response to PSC 14 Days Notice",
        "digitized_form_key": "comp_14d",
        "pdf": "4-Submission- Response to PSC 14 days Notice.pdf",
    },
    "COMP-OMB": {
        "name": "Ombudsman Request for Information from PSC",
        "digitized_form_key": "comp_omb",
        "pdf": "6-Submission- Ombudsman request of information from PSC.pdf",
    },
    "COMP-PSA": {
        "name": "Proposed Amendment to Public Service Act",
        "digitized_form_key": "comp_psa",
        "pdf": "1-Submission- Proposed Amendment to Public Service Act.pdf",
    },
}

COMPLIANCE_FORM_CODES = tuple(FORM_META.keys())
...
def fields_for_form_type(code):
    builders = {
        ...
        "COMP-OMB": lambda: [
            ...
            _fld(
                "Information requested (verbatim summary)",
...
<!--
  {meta['name']} ({code})
  OPSC-internal Compliance unit submission (not ministry HR).
  Import into Form Builder: Admin → Form Types → open form type → Import (.xml)
  Source PDF: {meta['pdf']}
  Digitized key: {meta['digitized_form_key']}
-->
<fields>
...
def main() -> None:
    ...
    for code in COMPLIANCE_FORM_CODES:
        ...
        for out_dir in OUTPUT_DIRS:
            ...
            created.append(path)
    print(f"Generated {len(created)} XML files ({len(COMPLIANCE_FORM_CODES)} forms × {len(OUTPUT_DIRS)} folders)")
